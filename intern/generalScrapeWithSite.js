...
(async () => {
    ...
    for (const [engineKey, engineValue] of Object.entries(searchEngines)) {
        try {
            ...
            if (!!jobData.general) {
                searchtab = "general";
                data = await extractLinks(engineKey, page);
            } else if (!!jobData.news) {
                searchtab = "news";
                data = await extractNewsLinks(engineKey, page);
            } else {
                console.log("fail");
    ...
    for (let i = 0; i < allData.maxLength; ++i) {
        ...
        csvString += `${
            i + 1
        },${googleData},${bingData},${duckData},${tuskData}\n`;
    }
    ...
    try {
        conn = await pool.getConnection();
        resp = await conn.query(
            `UPDATE jobs SET completed = ? WHERE jid = ?;`,
            [completed, jobData.jid]
        );
        console.log(resp);
