...
campaignRouter.get('/', async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < resp.length; ++i) {
            let total_resp = await conn.query('SELECT COUNT(cid) as count FROM jobs WHERE cid = ?', [resp[i].cid])
            let completed_resp = await conn.query('SELECT COUNT(cid) as count FROM jobs WHERE cid = ? and completed = 1', [resp[i].cid])
...
campaignRouter.delete('/:id', async (req, res) => {
    ...
    !sendErr && resp ? res.status(200).send() : res.status(400).json({ error: sendErr })
...
campaignRouter.post('/', async (req, res) => {
    ...
    try {
        conn = await pool.getConnection()
        resp = await conn.query('INSERT INTO campaigns (cname, link) VALUES (?, ?)',
            [
                campaignName,
                null
            ])
        ...
        for (let i = 0; i < jobs.length; ++i) {
            resp = await conn.query('INSERT INTO jobs (cid, query, siteurl, completed) VALUES (?, ?, ?, ?)',
                [
                    campaignID,
                    jobs[i].query,
                    jobs[i].site,
    ...
    !sendErr && resp ? res.status(201).send() : res.status(400).json({ error: sendErr })
    // res.status(201).send()
})
